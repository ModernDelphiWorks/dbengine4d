...
begin
  ...
  Assert.AreEqual(FDBConnection.InTransaction, FConnection.DefaultTransaction.InTransaction, 'FConnection.InTransaction <> FFDConnection.InTransaction');
...
begin
  ...
  FConnection.DatabaseName := '.\database.fdb';
  FConnection.Params.Clear;
  FConnection.Params.Add('user_name=SYSDBA');
  FConnection.Params.Add('password=masterkey');

  FDBConnection := TFactoryIBExpress.Create(FConnection, dnFirebird);
end;
